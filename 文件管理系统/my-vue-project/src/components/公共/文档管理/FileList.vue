...
export default {
    ...
    methods: {
        ...
        async updatelist() {
            ...
            var url = '';
            if (this.user.type) {
                url = 'http://192.168.0.101:8086/document/';
            }
            else {
                url = 'http://192.168.0.101:8086/document/user/' + this.user.id;
            }
        ...
        fetchData() {
            try {
                const res = axios.get('http://192.168.0.101:8086/document/');
                if (res.status === 200) {
                    console.log('http://192.168.0.101:8086/document/');
                ...
                if (!$.fn.DataTable.isDataTable('#example')) {
                    ...
                    this.$nextTick(() => {
                        ...
                        $('#example tbody').off('click', '.view-btn').on('click', '.view-btn', async (event) => {
                            ...
                            // 设置下载url
                            a.href = 'http://192.168.0.101:8086/document/view/' + user.id + '/' + documentId;
                            // 点击下载
                            a.click();
                        });
                        ...
                        $('#example tbody').off('click', '.edit-btn').on('click', '.edit-btn', (event) => {
                            ...
                            axios.get('http://192.168.0.101:8086/document/' + data).then(res => {
                        ...
                        $('#example tbody').off('click', '.download-btn').on('click', '.download-btn', async (event) => {
                            ...
                            var a = document.createElement('a');
                            // 设置下载url
                            a.href = 'http://192.168.0.101:8086/document/download/' + user.id + '/' + documentId;
                            // 点击下载
                            a.click();
                        });
                        ...
                        $('#example tbody').off('click', '.delete-btn').on('click', '.delete-btn', (event) => {
                            var user = JSON.parse(localStorage.getItem('user'));
                            axios.delete('http://192.168.0.101:8086/document/' + user.id + '/' + $(event.currentTarget).val()).then(res => {
                                if (res.data) {
                                    console.log('删除成功');
                                    alert('删除成功');
                                } else {
