    ...
    <div class="content-wrapper">
        <div class="container-fluid">
            ...
            <div class="row pt-2 pb-2">
                <div class="col-sm-9">
                    <h4 class="page-title" v-show="!user?.type">我的应用</h4>
                    <h4 class="page-title" v-show="user?.type">用户应用</h4>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">应用管理</li>
                        <li class="breadcrumb-item active" aria-current="page" v-show="!user?.type">我的应用</li>
                        <li class="breadcrumb-item active" aria-current="page" v-show="user?.type">用户应用</li>
                    </ol>
                </div>
            </div>
            ...
            <app-edit :initialapplication="initialapplication" v-if="initialapplication.id != ''" @data-back="handleDataBack" />

            <!-- End Breadcrumb-->
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header"><i class="fa fa-table"></i> 应用列表</div>

                        <div class="card-body">

                            <div class="table-responsive">
                                <table id="example-app" class="table table-bordered">
...
export default {
    ...
    methods: {
        ...
        async updatelist() {
            ...
            var url = '';
            if (this.user.type) {
                url = 'http://192.168.0.104:8086/application/';
            }
            else {
                url = 'http://192.168.0.104:8086/application/user/' + this.user.id;
            }
        ...
        fetchData() {
            try {
                const res = axios.get('http://192.168.0.104:8086/application/');
                if (res.status === 200) {
                    console.log('http://192.168.0.104:8086/application/');
                    console.log('this.applicationlist = res.data;');
                }
            } catch (err) {
                console.error(err);
            }
        ...
        async initializeDataTable(url) {
            ...
            var applicationlist2d = null;
            try {
                if (!$.fn.DataTable.isDataTable('#example-app')) {
                    ...
                    try {
                        ...
                        if (promise.status === 200) {
                            ...
                            applicationlist2d = this.convertTo2DArray(promise.data);
                        } else {
                            console.log(promise);
                            return;
                        }
                    ...
                    this.$nextTick(() => {
                        console.log(" var table = $('#example-app').DataTable({");
                        var table = $('#example-app').DataTable({
                        ...
                        // 绑定编辑、下载和删除按钮的事件
                        $('#example-app tbody').off('click', '.edit-btn').on('click', '.edit-btn', (event) => {
                            var data = $(event.currentTarget).val();
                            console.log('编辑数据:', data);
                            console.log('数据:', this.initialapplication);
                            axios.get('http://192.168.0.104:8086/application/' + data).then(res => {
                                this.initialapplication = res.data;
                            });

                        });

                        $('#example-app tbody').off('click', '.download-btn').on('click', '.download-btn', async (event) => {
                            var user = JSON.parse(localStorage.getItem('user'));
                            var applicationId = $(event.currentTarget).val();
                            // 创建一个<a>link标签
                            var a = document.createElement('a');
                            // 设置下载url
                            a.href = 'http://192.168.0.104:8086/application/download/' + user.id + '/' + applicationId;
                            // 点击下载
                            a.click();
                        });

                        $('#example-app tbody').off('click', '.delete-btn').on('click', '.delete-btn', (event) => {
                            var user = JSON.parse(localStorage.getItem('user'));
                            axios.delete('http://192.168.0.104:8086/application/' + user.id + '/' + $(event.currentTarget).val()).then(res => {
                                if (res.data) {
                                    console.log('删除成功');
                                    alert('删除成功');
                                } else {
                                    console.error('删除失败');
                                    alert('删除失败,你无权删除该应用或已删除未刷新');
                                }
                            });
                        });


                        console.log("table.buttons().container().appendTo(#example-app_wrapper.col-md-6:eq(0));");
                        table
                            .buttons()
                            .container()
                            .appendTo("#example-app_wrapper .col-md-6:eq(0)");
